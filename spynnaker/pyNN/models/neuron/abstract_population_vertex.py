...
class AbstractPopulationVertex(
    ...
    def _get_sdram_usage_for_neuron_params(self, vertex_slice):
        ...
        recording_size = ReceiveBuffersToHostBasicImpl.get_recording_data_size(
        ...
        return ((common_constants.DATA_SPECABLE_BASIC_SETUP_INFO_N_WORDS * 4) +
                recording_size + (per_neuron_usage * vertex_slice.n_atoms) +
    ...
    def _reserve_memory_regions(
        ...
        # Reserve memory:
        recording_size = subvertex.get_recording_data_size(
            3, [self._spikes_schedule, self._v_schedule, self._gsyn_schedule])
    ...
    def set_recording_v(self, schedule=None):
        if schedule is None:
            schedule = []
    ...
    def set_recording_gsyn(self, schedule=None):
        if schedule is None:
            schedule = []
